...
static void set_envvars(const Envvars* env);
...
static void start_executables(const Executables* exe);
		...
		double dx_unaccel, double dy_unaccel)
{
    ...
    if (!surface && !seat->drag) {
        struct wlr_xcursor *xc = wlr_xcursor_manager_get_xcursor(cursor_mgr, "default", 1);
        fprintf(stdout, "motionnotify: setting default cursor, xcursor=%p\n", xc);
        wlr_cursor_set_xcursor(cursor, cursor_mgr, "default");
    }
...
void set_envvars(const Envvars* env) {
    for (int i = 0; i < env->count; i++) {
        setenv(env->vars[i].name, env->vars[i].value, 1);
    }
}
...
setup(void)
{
    ...
    fprintf(stdout, "XCURSOR_THEME env: %s\n", getenv("XCURSOR_THEME") ? getenv("XCURSOR_THEME") : "NULL");
    fprintf(stdout, "XCURSOR_PATH env: %s\n", getenv("XCURSOR_PATH") ? getenv("XCURSOR_PATH") : "NULL");
    int loaded = wlr_xcursor_manager_load(cursor_mgr, 1);
    fprintf(stdout, "cursor_mgr load result: %d\n", loaded);
    struct wlr_xcursor *xcursor = wlr_xcursor_manager_get_xcursor(cursor_mgr, "left_ptr", 1);
    fprintf(stdout, "xcursor lookup result: %p\n", xcursor);
    if (xcursor) {
        fprintf(stdout, "xcursor name: %s\n", xcursor->name);
        fprintf(stdout, "xcursor image count: %d\n", xcursor->image_count);
    }
...
void start_executables(const Executables* exe) {
    for (int i = 0; i < exe->count; i++) {
        ...
        if (pid == -1) {
            ...
            execvp(exe->executables[i].args[0], (char* const*)exe->executables[i].args);
